...
with tab1:
    if not LABS or not EQUIPMENT:
        st.error("⚠️ [관리자] 랩/기기를 먼저 등록해주세요.")
    else:
        ...
        with col1:
            ...
            with st.form("booking_form"):
                ...
                if st.form_submit_button("예약 등록", use_container_width=True):
                    ...
                    if not u_name or len(u_pw) != 4: st.error("이름과 비밀번호(4자리) 필수")
                    elif not fs or not fe: st.error("시간 형식 오류 (예: 0900)")
                    elif fs == fe: st.error("시작/종료 시간이 같음")
                    else:
                        df_bk = load_data('bookings')
                        is_overnight = fe < fs
                        ...
                        if is_overnight:
                            ...
                            if ov: st.error(f"❌ 예약 충돌! ({usr}님이 사용중)")
                            else:
                                row = pd.DataFrame([{
...
with tab2:
    ...
    t_date = st.date_input("조회 날짜", datetime.now(), key="timeline_d")
    ...
    if EQUIPMENT:
        ...
        if not df_st.empty:
            ...
            df_st['mon'] = pd.to_datetime(df_st['date']).dt.strftime('%Y-%m')
            
            c1, c2 = st.columns(2)
            with c1:
                cur = datetime.now().strftime('%Y-%m')
                st.write(f"**이번 달 ({cur})**")
                df_cur = df_st[df_st['mon'] == cur]
                if not df_cur.empty:
                    base = alt.Chart(df_cur).encode(theta=alt.Theta("hrs", stack=True))
                    pie = base.mark_arc(innerRadius=60).encode(color=alt.Color("lab", scale=lab_scale), tooltip=['lab', 'hrs'])
                    st.altair_chart(pie, use_container_width=True)
                else: st.caption("데이터 없음")
            with c2:
                st.write("**월별 추이**")
                bar_d = df_st.groupby(['mon', 'lab'])['hrs'].sum().reset_index()
                bar = alt.Chart(bar_d).mark_bar().encode(x='mon', y='hrs', color=alt.Color('lab', scale=lab_scale)).properties(height=300)
...
with tab4:
    ...
    if apw == ADMIN_PASSWORD:
        st.success("접속 승인")
        at1, at2, at3, at4 = st.tabs(["⚙️ 랩/기기", "📅 예약DB", "💧 3차수DB", "📜 로그"])
        
        with at1:
            st.info("💡 **주의:** 표에서 이름을 직접 바꾸면 예약 내역은 바뀌지 않습니다. **아래 '일괄 변경' 도구를 사용하세요.**")
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("#### 🧪 실험실")
                d_lab = st.data_editor(load_data('labs'), num_rows="dynamic", key="ed_l", hide_index=True)
                if st.button("목록 저장", key="sv_l"): save_data('labs', d_lab); st.rerun()
                
                with st.expander("🔄 실험실 이름 일괄 변경 (데이터 연동)", expanded=True):
                    if LABS:
            ...
            with c2:
                ...
                if st.button("목록 저장", key="sv_e"): save_data('equipment', d_eq); st.rerun()
                
                with st.expander("🔄 기기 이름 일괄 변경 (데이터 연동)", expanded=True):
                    if EQUIPMENT:
