...
def load_data(data_type):
    try:
        ws = get_worksheet(st.secrets["spreadsheet_name"], data_type)
        if ws:
...
def batch_rename(target_type, old_name, new_name):
    data_type = 'labs' if target_type == 'lab' else 'equipment'
    df_master = load_data(data_type)
    
    if old_name in df_master['name'].values:
        df_master.loc[df_master['name'] == old_name, 'name'] = new_name
        save_data(data_type, df_master)
    ...
    if target_type == 'lab':
        ...
        if not df_water.empty:
            ...
            if mask_w.sum() > 0:
                df_water.loc[mask_w, 'lab'] = new_lab_name
                save_data('water', df_water)
    return True

# 동적 데이터 로드
@st.cache_data(ttl=60)
def get_labs_and_equipment():
    df_labs = load_data('labs')
    df_equipment = load_data('equipment')
...
LABS, EQUIPMENT = get_labs_and_equipment()
...
with tab1:
            ...
            if st.button("예약 등록하기", type="primary", use_container_width=True):
                ...
                if not user_name or len(password) != 4:
                    ...
                    if is_overnight:
                        ...
                        if overlap1: 
                            st.error(f"❌ 오늘 밤 예약 충돌! ({user1}님)")
                        elif overlap2: 
                            st.error(f"❌ 내일 새벽 예약 충돌! ({user2}님)")
                        else:
                        ...
                        if formatted_start == formatted_end: 
                            ...
                            if overlap: 
                                st.error(f"❌ 예약 충돌! ({overlap_user}님)")
                            else:
                                new_data = pd.DataFrame([{
    ...
    with col2:
        ...
        timeline = alt.Chart(chart_df).mark_bar(cornerRadius=5).encode(
            ...
            tooltip=['user_name', 'lab', alt.Tooltip('start_time', type='nominal', title='시작'), alt.Tooltip('end_time', type='nominal', title='종료')]
            ...
            if not df_stats.empty:
                ...
                with c1:
                    ...
                    if not df_this.empty:
                        ...
                        pie = base.mark_arc(innerRadius=60).encode(
                            ...
                            order=alt.Order("duration", sort="descending"), 
                            tooltip=["lab", "duration", alt.Tooltip("percent", format=".1%")]
                        )
                ...
                with c2:
                    ...
                    bar = alt.Chart(bar_data).mark_bar().encode(
                        ...
                        y='duration', 
                        color=alt.Color('lab', scale=lab_scale), 
                        tooltip=['month', 'lab', 'duration', alt.Tooltip('percent', format='.1%')]
                    ).properties(height=300)
                    st.altair_chart(bar, use_container_width=True)
        else: 
            st.info(f"'{stat_eq}'에 대한 예약 데이터가 없습니다.")
    else: 
        st.info("데이터가 없습니다.")
...
with tab3:
    ...
    with col2:
        ...
        if not df_w.empty:
            ...
            bar = alt.Chart(m_stats).mark_bar().encode(
                x='month', 
                y='amount', 
                color=alt.Color('lab', scale=lab_scale), 
                tooltip=['month', 'lab', 'amount', alt.Tooltip('percent', format='.1%')]
